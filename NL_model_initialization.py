"""
This script initializes the NL model's
utility functions.
Version: 28.9.2025
Ville Isoranta
"""
...
def model_initialization_func ():
    ...
    V_NL = {1: V_G , 2: V_S , 3: V_TRAN }  # Joins the choice to the utility function

    av = {1: 1, 2: 1, 3:COMM_AVAI_TRAN}    # Sets an availability condition

    """
    Creates the nests as discussed in Appendix J
    For clarity also the trivial nest tran is included.
    """
    car_nest= OneNestForNestedLogit(nest_param=MU, list_of_alternatives=[1,2], name='car')
    tran_nest= OneNestForNestedLogit(nest_param=1, list_of_alternatives=[3], name='tran')
    nest = NestsForNestedLogit(choice_set=list(V_NL), tuple_of_nests=(car_nest,tran_nest))
